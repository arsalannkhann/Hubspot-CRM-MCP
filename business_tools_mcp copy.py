...
async def handle_list_tools() -> List[types.Tool]:
    """List only the business tools that are properly configured via environment.
    This avoids exposing tools that will error due to missing credentials/config.
    """
    tools: List[types.Tool] = []

    # Helper to append tools
    def add_web_search():
    ...
    def add_social_media_post():
        tools.append(types.Tool(
            name="social_media_post",
            description="Post to Twitter/X (LinkedIn removed)",
            inputSchema={
                "type": "object",
                "properties": {
                    "platform": {"type": "string", "enum": ["twitter", "x"], "description": "Only Twitter/X supported"},
                    "content": {"type": "string", "description": "Post content (max 280 chars for Twitter)"},
                    "media": {"type": "array", "items": {"type": "string"}, "description": "Media URLs (optional)"},
                    "dry_run": {"type": "boolean", "description": "Test mode - validate credentials without posting", "default": False}
                },
                "required": ["platform", "content"]
            }
        ))

    # Decide which tools to expose based on configuration
    # 1) Web search
    if (SERPAPI_KEY and SERPAPI_KEY != "your_serpapi_key_here") or (GOOGLE_CUSTOM_SEARCH_KEY and GOOGLE_CUSTOM_SEARCH_CX):
        add_web_search()

    # 2) Database query (requires DATABASE_URL)
    if DATABASE_URL:
        add_database_query()

    # 3) CRM operation (HubSpot token)
    if HUBSPOT_TOKEN:
        add_crm_operation()

    # 4) Enrichment (Clearbit or People Data Labs)
    if CLEARBIT_KEY or PEOPLE_DATA_LABS_KEY:
        add_enrich_data()

    # 5) Calendar operation (Google token file/env or Outlook creds)
    google_token_file = os.getenv("GOOGLE_CALENDAR_TOKEN", "google_calendar_token.pickle")
    if (os.path.exists(google_token_file)) or (os.getenv("OUTLOOK_CLIENT_ID") and os.getenv("OUTLOOK_CLIENT_SECRET")):
        add_calendar_operation()

    # 6) Twilio
    if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN:
        add_twilio_communication()

    # 7) Email (SendGrid or Gmail)
    if SENDGRID_KEY or (GMAIL_EMAIL and GMAIL_APP_PASSWORD):
        add_send_email()

    # 8) Stripe
    if STRIPE_KEY:
        add_stripe_operation()

    # 9) Docs (Notion or Google Drive)
    if NOTION_KEY or GOOGLE_DRIVE_KEY:
        add_docs_operation()

    # 10) Social media (Twitter/X)
    ...
    if mongo_client and (DATABASE_URL and DATABASE_URL.startswith("mongodb")):
        ...
        try:
            ...
            if "find" in query or "select" in query or "get" in query:
                ...
                cursor = coll.find(filter_doc, projection) if projection else coll.find(filter_doc)
                if sort:
        ...
        try:
            if operation == "create_contact":
                ...
                response = hubspot_client.crm.deals.basic_api.create(
                    DealInput(properties=properties)
                )
                return [types.TextContent(
                    type="text",
                    text=json.dumps({
...
async def send_email_tool(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    # Mailgun
        ...
        return [types.TextContent(
            type="text",
            text=json.dumps({
                "error": "Mailgun provider not supported in this version",
                "tool": "send_email"
            })
        )]
    ...
    if provider == "notion":
        ...
        try:
            if action == "search":
                ...
                if not parent_id and not data.get("parent"):
                    return [types.TextContent(
                        type="text",
                        text=json.dumps({
                            "error": "Either parent_id or parent must be provided to create a page",
                            "tool": "docs_operation"
                        })
                    )]
                ...
                new_page = await notion_client.pages.create(
                    parent = data.get("parent") or {"database_id": parent_id},
                    properties={
                        "title": {
                            "title": [
                                {
    ...
    if platform.lower() in ["twitter", "x"]:
        ...
        try:
            ...
            if args.get("dry_run", False):
                try:
                    user = client.get_me()
                    username = user.data.username if user and getattr(user, "data", None) else None
                except:
                    username = None
...
async def _async_cleanup():
    """Run async cleanup tasks. Safe to call inside an asyncio loop."""
    # Try shutting down the MCP app cleanly (if supported)
    try:
        await app.shutdown()
    except Exception as e:
        logger.exception("app.shutdown() raised during cleanup: %s", e)

    # Run any project-specific async cleanup (if defined)
    try:
        if 'cleanup_clients' in globals() and callable(globals()['cleanup_clients']):
            await cleanup_clients()
    except Exception as e:
        logger.exception("cleanup_clients failed during shutdown: %s", e)

    logger.info("Async cleanup finished.")

def _sync_cleanup():
    """Run synchronous cleanup after KeyboardInterrupt. Does not mass-cancel tasks."""
    # Close MongoDB if open
    try:
        ...
        # defensive; don't blow up during cleanup
        pass

    logger.info("Sync cleanup finished.")
...
async def _main():
    """Main async function to run the MCP server with proper stdio transport"""
